...
class Watermark
  ...
  attr_reader :originals_directory,
  ...
  def initialize(key)
    ...
    @originals_directory = connection.
              directories.get(ENV['AWS_S3_BUCKET_ORIGINALS'])
    @watermarked_directory =
      connection.directories.get(ENV['AWS_S3_BUCKET_WATERMARKED'])
  ...
  def apply_watermark
    Dir.mktmpdir do |tmpdir|
      ...
      result =
        image.composite(MiniMagick::Image.open('watermark.png', 'jpg')) do |c|
          c.dissolve '15'
          c.gravity 'center'
        end
  ...
  def save_watermarked_file(watermarked_local_file)
    ...
    flush 'Persisted watermarked file to S3:' +
          " #{watermarked_file_token.public_url}"
  end
