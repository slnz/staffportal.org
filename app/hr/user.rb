ActiveAdmin.register User, :namespace => :hr do
  ...
  controller do
    def resource
      @user ||= end_of_association_chain.where("bootcamp_coach_id IS NOT NULL and id = ?", params[:id]).first!
    end

    def collection
      @users ||= end_of_association_chain.where("bootcamp_coach_id IS NOT NULL").page(params[:page]).per(10)
    end
  end
