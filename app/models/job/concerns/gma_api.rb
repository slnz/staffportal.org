    ...
    module GmaApi
      ...
      def publish(db_gma_report)
        ...
        unless auth == false
          ...
          response = HTTParty.put('https://www.oceaniaccc.org/Oceania/',
      ...
      def import_user(user, auth=false)
        ...
        unless auth == false
          response = HTTParty.get('https://www.oceaniaccc.org/Oceania/',
          ...
          gma_orgs['data']['director'].each do |gma_org|
            db_gma_org = GmaOrganization.where(gma_id: gma_org['nodeId']).first_or_create
          ...
          response = HTTParty.post('https://www.oceaniaccc.org/Oceania/',
          ...
          gma_reports['data']['staffReports'].each_with_index do |gma_report, index|
            at(index,
               ...
               "#{index} of #{gma_reports['data']['staffReports'].count} Staff Reports Imported!")
            db_gma_org = GmaOrganization.where(gma_id: gma_report['node']['nodeId']).first_or_create
            ...
            db_gma_report.update(user_id: user.id,
                                 gma_organization_id: db_gma_org.id,
            ...
            gma_measurements['data']['numericMeasurements'].each do |hash|
              hash.each do |strategy, measurements|
                measurements.each do |measurement|
                  db_measurement = GmaMeasurement.where(gma_id: measurement['measurementId'],
                                                        gma_organization_id: db_gma_org.id,
                                                        gma_staff_report_id: db_gma_report.id).first_or_create
      ...
      def authenticate(user)
        ...
        # https://github.com/GlobalTechnology/php-gma-api-client/blob/master/GMA_API.class.php
        ...
        # parse tgt
        tgtUrl = response.header['location']

        # return if login failure
        if response.code != 201 || tgtUrl.blank?
          return false
        end

        # get ST
        response = HTTParty.post(tgtUrl,
                                 body: { service: 'https://www.oceaniaccc.org/Oceania/?q=node&destination=node' })

        # parse ticket
        ticket = response.body

        # return if ticket failure
        if response.code != 200 || ticket.blank?
          return false
        end

        # use ST w/ GMA (follow redirects & capture cookies)
        response = HTTParty.get("https://www.oceaniaccc.org/Oceania/?q=node&destination=node&ticket=#{ticket}")

        # extract the correct Set-Cookie header
        cookie = response.request.options[:headers]['Cookie']

        response = HTTParty.get('https://www.oceaniaccc.org/Oceania/?q=services/session/token',
